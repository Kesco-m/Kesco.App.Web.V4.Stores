...
namespace Kesco.App.Web.Stores
{
    ...
    public class StoresPageHelper
    {
        ...
        private static readonly StringCollection OldParameterNamesCollection = new StringCollection
        {
            ...
            "StoreKindType" //группы типов складов
        };
        ...
        private readonly AppParamsManager _parametersManager;
        ...
        private readonly HashSet<string> _requestParameters;
        ...
        public static string GetStrHiddenFields(List<string> visible, ref string[] hiddenFields)
        {
            var index = 0;
            hiddenFields = new string[_searchFields.Length / _searchFields.Rank];

            var sb = new StringBuilder("[");
            foreach (var s in _searchFields)
        ...
        ///     Возвращаемая строка представления массива идентификаторов обязательных полей поиска для
        ///     передачи его в Javascript код
        /// </param>
        /// <returns>Строковое представления массива для передачи его в Javascript код</returns>
        public string GetFormHiddenFields(string strVisible, ref string[] hiddenFields, ref string requiredFields)
        {
            string[,] p =
            {
                {_searchFields[0], SearchParameters.ValidMode},
            ...
            var visible = new List<string>(strVisible.Split(' ', ','));

            var sb = new StringBuilder("[");

            for (var i = 0; i < p.Length / p.Rank; i++)
        ...
        public void setTextBoxParameterValue(TextBox ctrl, string param_mode, string param_value)
        {
            ...
            var value = getParameterValue(param_value, out isRequiredValue, string.Empty);
            var mode = getParameterValue(param_mode, out isRequiredMode,
                (txtModeOffset + (int) TextBoxEnum.ContainsAll).ToString());

            setTextBoxValue(ctrl, mode, value, false);
        }
        ...
        /// <param name="requiredIsReadOnly">Флаг указывающий, что значение параметра обязятельное, но редактирование не запрещено</param>
        public void setTextBoxValue(TextBox ctrl, string mode, string value, bool isRequired,
            bool requiredIsReadOnly = true)
        {
            var intMode = 0;
            ...
            var modes = (TextBoxEnum[]) Enum.GetValues(typeof(TextBoxEnum));
            ...
            if (!ctrl.IsReadOnly && requiredIsReadOnly) ctrl.IsReadOnly = isRequired; // ctrl.IsDisabled;
            if (ctrl.HasCheckbox)
            ...
            if (arr.Length > 0)
            {
                if (ctrl.IsMultiSelect)
                    foreach (var id in arr)
                    {
                        ...
                        if (obj != null)
                            ctrl.SelectedItems.Add(new Item {Id = id, Value = obj});
                    }
                else
                    ctrl.Value = arr[0];
            }
        ...
        ///     Возращаемый тип полученного значения 0-значение по умолчанию defstr, 1-из строки запроса, 2 -
        ///     из БД
        /// </param>
        ...
        {
            var value = _request.QueryString[name];
        ...
        {
            var appParam = _parametersManager.Params.Find(p => p.Name == name);
        ...
        public static class OldParameters
        {
            ...
            public static readonly string KindType = OldParameterNamesCollection[12];
        }
        ...
        public static class StoreParameters
        {
            ...
            public static readonly string KindType = StoreParameterNamesCollection[14];
            public static readonly string Search = StoreParameterNamesCollection[15];
        }
