...
function CancelStoresOrder() {
    ...
    if (HasChanges()) {
        CustomConfirmChangedTwoButtons.render("",
            StrResources.MsgCancelConfirm,
            StrResources.BtnYes,
            StrResources.BtnNo);
    } else
    ...
    var fn_forEach = function(item, i, arr) {
        var checkbox =
            document.getElementById(
                item[0]); // document.getElementById(i); //для OnCheckStore.selected_stores i = store_id
        if (checkbox) {
            checkbox.checked = true;
...
function OnCheckStore() {
    ...
    if (checked_rows.length > 0) {
        droppable_obj.droppable({
            accept: dragged_table,
            scope: "report",
            tolerance: "touch",
            drop: onDropDraggedRows
        });
        ...
        var drag_area = [
            droppable_el.offset().left, droppable_el.offset().top, droppable_el.offset().left + droppable_el.width(),
            droppable_el.offset().top + droppable_el.height() /* + checked_rows_height*/
        ];
        dragged_table.draggable({
            appendTo: droppable_el,
            axis: "y",
            handle: checked_rows,
...
function deleteSelected() {
    ...
    CustomConfirmChangedTwoButtons.render("",
        StrResources.MsgDeleteSelectedStoresConfirm,
        StrResources.BtnYes,
        StrResources.BtnNo);
