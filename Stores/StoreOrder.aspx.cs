...
namespace Kesco.App.Web.Stores
{
    ...
    public partial class StoreOrder : EntityPage
    {
        ...
        private void SetMenuButtons()
        {
            ...
            Button[] buttons = {btnOk, btnSave, btnCancel, btnClose};
        ...
        protected override void ProcessCommand(string cmd, NameValueCollection param)
        {
            ...
            switch (cmd)
            {
                ...
                */
                ...
                {
                    var page = param["page"].ToInt();
                    var page_size = param["page_size"].ToInt();
                    var strReportType = param["report_type"];
                    //if (null == strReportType) strReportType = sStoreReportType.Value;
                    var reportType = strReportType.ToInt();
        ...
        private void RemoveStores(string strStores)
        {
            ...
            parameters.Add("@КодыСкладов", strStores);
            parameters.Add("@КодТипаОтчётаПоСкладам",
                string.IsNullOrEmpty(sStoreReportType.Value) ? DBNull.Value : (object) sStoreReportType.Value);

            try
            {
                DBManager.ExecuteNonQuery(SQLQueries.DELETE_СкладыИзОтчётаПоСкладам, CommandType.Text, Config.DS_person,
                    parameters);
        ...
        private void ApplyStoresOrder(int reportType, string strStores, bool fAtBottomOfPage, int toPage, int pageSize)
        {
            ...
            var storesCount = originalStoresId.Length / originalStoresId.Rank;

            if (pageSize < 1) pageSize = storesCount;

            var new_stores = strStores.Split(' ', ',');
            var new_stores_id = new List<int>();
            foreach (var s in new_stores)
            {
                var id = s.ToInt();
            ...
            try
            {
                ...
                //if(dtStoresResult.Equals(prevStoresResult))
                {
                    ...
                    var new_stores_order = new int[storesCount];
                    var new_index = 0;
                    ...
                    while (i < storesCount && new_index < storesCount)
                    {
                        if (new_index < nStart)
                        {
                            ...
                            var store_id = originalStoresId[i++, 0];

                            if (!new_stores_id.Contains(store_id)) new_stores_order[new_index++] = store_id;
                        }
                        ...
                        if (new_index >= nStart)
                        {
                            ...
                            foreach (var id in new_stores_id) new_stores_order[new_index++] = id;
                    ...
                    if (new_index > 0)
                    {
                        ...
                        var diff = new int[new_index];

                        for (var j = 0; j < new_stores_order.Length; j++)
                            ...
                            if (originalStoresId.Length <= j || originalStoresId[j, 1] != j + 1 ||
                                originalStoresId[j, 0] != new_stores_order[j])
                        ...
                        if (changed_count > 0)
                        {
                            ...
                            DBManager.ExecuteNonQuery(string.Format(SQLQueries.UPDATE_СкладОтчётыПоСкладам, sb),
                                CommandType.Text, Config.DS_person, parameters);
        ...
        {
            var dtStoresResult = _table.Update();
            if (null != dtStoresResult && dtStoresResult.Rows.Count > 0)
            {
                originalStoresId = new int[dtStoresResult.Rows.Count, 2];
                for (var i = 0; i < dtStoresResult.Rows.Count; i++)
                {
                    var r = dtStoresResult.Rows[i];
                    var store_id = (int) r["КодСклада"];
                    var store_order = (int) r["Порядок"];
